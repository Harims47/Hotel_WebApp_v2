...
- [ ] **Input Validation:** API inputs are validated using Zod / Fastify schemas. Special characters are parameterized, and request body size is capped at 2MB at the reverse proxy (Nginx).
...
- [ ] **Schema Migrations:** Node.js schema migrations run and succeed on a clean PostgreSQL instance.
...
- [ ] **Nginx Reverse Proxy:** Nginx is configured to route traffic to the Vite static server and Fastify backend.
