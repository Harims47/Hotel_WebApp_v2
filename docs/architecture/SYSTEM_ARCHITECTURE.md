...
graph TD
    ...
    Proxy -->|REST API Requests| Fastify[Node.js / Fastify Backend Application]
    Fastify -->|Service Layer| Services[Service & Business Logic Layer]
    Services -->|pg driver| PostgreSQL[(PostgreSQL Database)]
...
- **Application Layer (Node.js Backend):** Provides high-performance async endpoint routing (Fastify), input validation, and acts as the central business logic controller.
...
| **Backend Framework** | Fastify (Node.js) | High performance (async native), lightweight architecture matching the modern Node ecosystem, and highly extensible via plugins. |
...
| **Database Access** | `pg` (Raw Driver) | Direct control over connection pooling, RLS context injection, and high-performance querying without ORM overhead. |
| **Database Migrations**| Custom Node Scripts | Idempotent schema migration scripts managed natively within the Node backend repository. |
...
| **Payload Validation** | Zod / Fastify Schema | Extremely fast parsing, type-safe serialization, and custom validator logic that bridges HTTP payloads and TypeScript object layers. |
...
- **Database Enforcement:** 
  - Database queries automatically append `WHERE restaurant_id = $current_restaurant_id AND location_id = $current_location_id` filtering (via RLS or explicit clauses) for operations except for Super Admin views.
...
- **Real-Time Communication:** Fast-changing operational events (e.g., KOT ready, new bill request) are pushed to clients using **WebSockets** managed by Fastify.
...
- **Timer Tasks:** Background tasks (e.g., KOT delay monitoring, automatic pickup escalation, expired session cleanup) are offloaded to a background worker system (e.g., Node.js intervals for simple loops, or BullMQ for production-grade scheduling).
...
- **Trigger Points:** Interceptors or centralized service layers automatically push audit records to this table, decoupling audit logging from application controllers.
