...
export function AdjustmentNew() {
  ...
  useEffect(() => {
    if (isGM) { toast.error('No permission'); navigate('/inventory/adjustments'); }
  }, [isGM, navigate]);
  ...
  useEffect(() => {
    if (editId && !existingAdjustment) { toast.error('Not editable'); navigate('/inventory/adjustments'); }
  }, [editId, existingAdjustment, navigate]);
  ...
  // Actual system stock for a given item+location from Redux state (read-only).
  ...
  const [adjItems, setAdjItems] = useState(() => {
    if (existingAdjustment) {
      return existingAdjustment.items.map(i => ({
        id: i.id,
        itemId: i.itemId,
  ...
  const handleAdd = () => setAdjItems([...adjItems, makeNewRow()]);
  const handleRemove = id => setAdjItems(adjItems.filter(i => i.id !== id));

  // When item changes, auto-populate rate from item.currentRate.
  // physicalQuantity resets to empty so the user must enter a count.
  const handleChange = (id, field, value) => {
    setAdjItems(adjItems.map(ai => {
      if (ai.id !== id) return ai;
      if (field === 'itemId') {
        return { ...ai, itemId: value, unitRate: getItemRate(value), physicalQuantity: '' };
      }
      return { ...ai, [field]: value };
    }));
  };

  // Diff = Physical - System. Returns 0 if physical not entered yet.
  const calcDiff = (ai) => {
    if (ai.physicalQuantity === '' || ai.physicalQuantity === null || ai.physicalQuantity === undefined) return 0;
    const sys = getSystemStock(ai.itemId, header.locationId);
    const physical = parseFloat(ai.physicalQuantity);
  ...
  const calcAmount = (ai) => {
    const diff = Math.abs(calcDiff(ai));
    const rate = parseFloat(ai.unitRate) || 0;
    return diff * rate;
  };
  ...
  const validate = () => {
    ...
    if (!header.reason) { toast.error('Please select an adjustment reason.'); return false; }
    if (!header.adjustmentDate) { toast.error('Adjustment Date is required.'); return false; }
    if (adjItems.length === 0) { toast.error('Add at least one item to adjust.'); return false; }
    for (const item of adjItems) {
      ...
      if (item.physicalQuantity === '' || item.physicalQuantity === null || item.physicalQuantity === undefined) {
        ...
        toast.error(`Please enter the physical quantity for ${name}.`);
        return false;
      }
      ...
      if (diff === 0) {
        const name = items.find(i => i.id === item.itemId)?.name || 'item';
        toast.error(`No stock variance exists for ${name}. Physical quantity matches system quantity.`);
        return false;
      }
    }
    return true;
  ...
  const handleSave = async (confirm) => {
    ...
    const newAdj = {
      id: newId, adjustmentNumber: adjNumber, ...header, status: 'DRAFT',
    ...
    dispatch(logAction({
      id: `log-${uuidv4()}`, userId: currentUser?.id, action: 'ADJUSTMENT_CREATED',
      entityType: 'ADJUSTMENT', entityId: newId, description: `Created Adjustment ${adjNumber}`, createdAt: timestamp,
    }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={editId ? `Edit Adjustment ${existingAdjustment?.adjustmentNumber || ''}` : 'New Adjustment'}
        breadcrumbs="Inventory / Adjustments / New"
        actions={<>
          ...
          <Button onClick={() => handleSave(true)}>
            <CheckCircle className="w-4 h-4 mr-2" />Confirm Adjustment
          </Button>
        </>}
      />
            ...
            <CardContent>
              ...
              <div className="md:hidden space-y-4">
                {adjItems.map((ai, idx) => {
                  ...
                  const diff = calcDiff(ai);
                  const rate = parseFloat(ai.unitRate) || 0;
                  const amount = calcAmount(ai);
                  const hasPhysical = ai.physicalQuantity !== '' && ai.physicalQuantity !== null;
                  return (
                    <div key={ai.id} className="border border-border rounded-xl p-4 space-y-3 bg-white">
                      <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => handleRemove(ai.id)} className="p-1.5 text-text-muted hover:text-status-danger">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                      <div>
                        <label className="text-xs text-text-muted block mb-1">Item *</label>
                        <Select label="Item" hideLabel value={ai.itemId}
                          onChange={e => handleChange(ai.id, 'itemId', e.target.value)}
                          options={[{ value: '', label: 'Select item...' }, ...activeItems.map(i => ({ value: i.id, label: i.name }))]}
                        />
                      </div>
                      {ai.itemId && (
                        <p className="text-xs text-text-muted">
                          System Qty: <span className="font-medium">{sys}</span>
                        </p>
                      )}
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="text-xs text-text-muted block mb-1">Physical Qty *</label>
                          <Input label="Physical Qty" hideLabel type="number" min="0" step="0.1"
                            value={ai.physicalQuantity}
                            onChange={e => handleChange(ai.id, 'physicalQuantity', e.target.value)}
                          />
                        </div>
                        <div>
                      ...
                      <div className="flex justify-between items-center pt-1">
                        <div className="text-xs">
                          Diff:{' '}
                          {ai.itemId && hasPhysical
                            ? <span className={`font-bold ${diff > 0 ? 'text-green-600' : diff < 0 ? 'text-red-500' : 'text-text-muted'}`}>
                                {diff > 0 ? `+${diff}` : diff}
                              </span>
                        ...
                        <div className="font-medium text-sm">
                          {ai.itemId && hasPhysical && rate > 0 ? formatCurrency(amount) : <span className="text-text-muted">—</span>}
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="hidden md:block overflow-x-auto">
                <Table>
                  <thead><tr>
                    ...
                    <Table.Th>Amount (Abs)</Table.Th>
                    <Table.Th align="right"></Table.Th>
                  </tr></thead>
                  <tbody>
                    {adjItems.length === 0
                      ? <tr><td colSpan="7" className="text-center py-8 text-text-muted">Add items to adjust</td></tr>
                      : adjItems.map(ai => {
                          const sys = getSystemStock(ai.itemId, header.locationId);
                          const diff = calcDiff(ai);
                          const rate = parseFloat(ai.unitRate) || 0;
                          ...
                          return (
                            <tr key={ai.id}>
                              ...
                              {/* System Qty is read-only — comes from actual location stock */}
                              <Table.Td>
                                <span className="font-medium">{ai.itemId ? sys : '—'}</span>
                              </Table.Td>
                              ...
                              {/* Diff is read-only — auto-calculated */}
                              <Table.Td>
                                {ai.itemId && hasPhysical
                                  ? <span className={`font-bold ${diff > 0 ? 'text-green-600' : diff < 0 ? 'text-red-500' : 'text-text-muted'}`}>
                                      {diff > 0 ? `+${diff}` : diff}
                              ...
                              {/* Rate is read-only — derived from item.currentRate */}
                              <Table.Td>
                                {ai.itemId
                                  ? rate > 0
                              ...
                              <Table.Td>
                                {ai.itemId && hasPhysical && rate > 0
                                  ? <span className="font-medium">{formatCurrency(amount)}</span>
                                  : <span className="text-text-muted">—</span>
                                }
                              </Table.Td>
                              <Table.Td align="right">
                                <button onClick={() => handleRemove(ai.id)} className="p-2 text-text-muted hover:text-status-danger">
                                  <Trash2 className="w-4 h-4" />
                                </button>
                              </Table.Td>
                            </tr>
            ...
            <CardContent>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{adjItems.length}</span>
                </div>
